...
const handleGetRequest = (url) => {
  ...
  if (cleanUrl.startsWith('/products')) {
    ...
    return db.getProducts(page, size, sortBy, sortDir, categoryId, minPrice, maxPrice);
  }
...
const API = {
  ...
  get: async (url, config) => {
    await delay();
    try {
      const responseData = handleGetRequest(url);
  ...
  post: async (url, data, config) => {
    await delay();
    try {
      const responseData = handlePostRequest(url, data);
      return { data: responseData };
    } catch (error) {
